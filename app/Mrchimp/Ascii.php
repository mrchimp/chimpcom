 ...
 * ASCII ART GENERATOR
 *
 * I, Jonathan Ford, release this into the public domain, in the hope that
 * someone, somewhere, may find it useful or interesting. As such, it comes
 * WITHOUT WARRANTY, without even the implied warranty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.
 *
 * However, if you do find a use for this, please try and get in touch.
 * An email, IM or even a link to my site would be nice :-).
 *
 * Requirements: PHP 5 or greater, a fairly recent version of GD (with GIF
 * support)
 *
 * Notes: I know this class isn't very efficient, or even properly OOP.
 * It does the job, though. If you're having problems opening images,
 * allow_url_fopen needs to be turned on in php.ini. For details of how to do
 * this see http://uk.php.net/manual/en/ref.filesystem.php#ini.allow-url-fopen.
...
class Ascii
{
    ...
    protected $image;
    protected $url;
    protected $size = 2; // Default size
    protected $quality = 2; // Default quality
    protected $color = false; // Default color/no color (true/false)
    // Characters to use when generating a B/W picture, in order from darkest to lightest
    protected $chars = array('@', '#', '+', '\'', ';', ':', ',', '.', '`', ' ');
    protected $color_char = '#'; // The block character for color pictures
    // Maximum filesize of image in bytes. Make sure you keep this below the memory_limit specified in php.ini
    ...
    public function __construct($url = 'http://ascii.techhappens.com/tux.gif')
    {
        ...
        // Rather messy way of checking if the request went OK. Needs sorting sometime.
        if (strstr($this->headers[0], '200') !== false) {
            // Check that the file isn't too big
            if ($this->headers['Content-Length'] < self::MAX_FILESIZE) {
                // Makes sure that a content type was specified
                if ($this->isImage($this->headers['Content-Type']) !== false) {
                    // Pretty self-explanatory - figure out which sort of image
                    // we're going to be processing and let GD know
     ...
     */
    public function isImage(string $content_type): bool
    {
        switch ($content_type) {
            case image_type_to_mime_type(IMAGETYPE_GIF):
            case image_type_to_mime_type(IMAGETYPE_JPEG):
     ...
     */
    public function draw(string $img = ''): string
    {
        if (empty($img) === true) {
        ...
        // I suppose this probably should be stuck into an array before we sort out the output
     ...
     */
    public function rgbtohex(string $red, string $green, string $blue): string
    {
        $hex = '#';
